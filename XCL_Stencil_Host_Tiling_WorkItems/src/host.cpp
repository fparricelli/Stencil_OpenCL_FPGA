...
int main ( int argc, char** argv ) {
    ...
    OCL_CHECK(err, err = q0.enqueueNDRangeKernel(rd_kernel, 0, 16, 16));//, 0, DATA_SIZE, DATA_SIZE));
    DEBUG("Running <compute>...");
    OCL_CHECK(err, err = q0.enqueueNDRangeKernel(cu_kernel, 0, 16 , 16 ));//, 0, DATA_SIZE, DATA_SIZE));
    DEBUG("Running <write_output>...");
    OCL_CHECK(err, err = q0.enqueueNDRangeKernel(wr_kernel, 0, 16, 16));//, 0, DATA_SIZE, DATA_SIZE));
