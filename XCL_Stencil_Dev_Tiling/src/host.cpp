...
#define PRINT_MATRIX 0              //!< 0: No matrix is printed after the kernel execution; 1: Expected and Output matrices are printed after the kernel execution 
...
#define ARRAY_SIZE  4096             //!< Array size referring to the number of elements 
#define COLS       128     
#define ROWS       128 
#define TIME_ITER 1
...
static signed   k4              = -4; 
/*!
 * @brief   Help function used to print an array of <ARRAY_SIZE> size
 * @param   input is the pointer to the memory location of the array
 * @retval  None
 */
template<typename array_t>
void print_array(array_t* input){
...
int main ( int argc, char** argv ) {
	...
	float array_out[ROWS][COLS]     __attribute__((aligned(64)));
    //---------------------------------------------------------------
    ...
    print_array<float>(&array_in_1[0][0]);
    ...
    size_t input_size_bytes = sizeof(int) * size;  //Byte-size of input-arrays
    size_t output_size_bytes = input_size_bytes;        //Byte-size of output-arrays
    ...
    OCL_CHECK(err, err = rd_kernel.setArg(1, rows));
    OCL_CHECK(err, err = rd_kernel.setArg(2, cols));
    OCL_CHECK(err, err = rd_kernel.setArg(3, input_size_bytes));

    OCL_CHECK(err, err = cu_kernel.setArg(0, input_size_bytes));
    OCL_CHECK(err, err = cu_kernel.setArg(1, rows));
    OCL_CHECK(err, err = cu_kernel.setArg(2, cols));
    OCL_CHECK(err, err = cu_kernel.setArg(3, buffer_output));
    
    OCL_CHECK(err, err = wr_kernel.setArg(0, buffer_output));
    OCL_CHECK(err, err = wr_kernel.setArg(1, rows));
    OCL_CHECK(err, err = wr_kernel.setArg(2, cols));
    OCL_CHECK(err, err = wr_kernel.setArg(3, input_size_bytes));
    //------------------------------------------------------------------
    ...
    if(verify<float>(&expected[0], &array_out[0][0])) std::cout << GREEN << "Test Passed";  else std::cout << RED << "Test NOT Passed";
    std::cout << RESET << std::endl;
#if PRINT_MATRIX == 1
    std::cout << "Printing expected array...\n";
    print_array<float>(&expected[0]);
    std::cout << "Printing obtained array...\n";
    print_array<float>(&array_out[0][0]);
#endif
    //------------------------------------------------------------------
	return EXIT_SUCCESS;
}
